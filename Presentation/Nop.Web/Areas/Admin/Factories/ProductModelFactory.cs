...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        public virtual async Task<ProductListModel> PrepareProductListModelAsync(ProductSearchModel searchModel)
        {
            ...
            var model = await new ProductListModel().PrepareToGridAsync(searchModel, products, () =>
            {
                return products.SelectAwait(async product =>
                {
                    ...
                    if (product.ProdCateId > 0)
                    {
                        Category categorymodel = await _categoryService.GetCategoryByIdAsync((int)product.ProdCateId);
                        productModel.Category = categorymodel.Name;
                    }
                    ...
                    // Added by Yogesh K on Dt: 09/30/2025
                    if (product.IsShipEnabled && product.Weight > 0)
                        productModel.Completeness = "Yes";
                    else
                        productModel.Completeness = "No";
        ...
        /// The task result contains the product list model
        /// </returns>
        public virtual async Task<ProductListModel> InstalledDocumentProductListModelAsync(ProductSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            //get products
            var products = await _productService.ProductSearchAsync(showHidden: true,
                categoryIds: categoryIds,
                manufacturerIds: new List<int> { searchModel.SearchManufacturerId },
                storeId: searchModel.SearchStoreId,
                vendorId: searchModel.SearchVendorId,
            ...
            var model = await new ProductListModel().PrepareToGridAsync(searchModel, products, () =>
            {
                return products.SelectAwait(async product =>
                {
                    ...
                    productModel.FileName = product.InstructionFileName;
                    productModel.FilePath = product.InstructionFilePath;
                    productModel.CompleteFileName = "/ProductInsructionsFile/"+ product.InstructionFilePath;
                    productModel.ProductName = product.Name;

                    if (product.ProductType == ProductType.SimpleProduct && product.ManageInventoryMethod == ManageInventoryMethod.ManageStock)
                        productModel.StockQuantityStr = (await _productService.GetTotalStockQuantityAsync(product)).ToString();

                    if (product.ProdCateId != null)
                    {
                        if (product.ProdCateId > 0)
                        {
                            int ProdCateInt = Convert.ToInt32(product.ProdCateId);
                            var MyListCat = await _categoryService.GetCategoryByIdAsync(ProdCateInt);

                            productModel.Category = MyListCat.Name;
                            productModel.CategoryId = Convert.ToInt32(product.ProdCateId);
                        }
                    }
                    else
                    {
        ...
        public virtual async Task<ProductModel> PrepareProductModelAsync(ProductModel model, Product product, bool excludeProperties = false)
        {
            ...
            else 
            {
                ...
                var allCategoriesProd1 = _categoryService.GetAllCategoriesInstruction();
                foreach (var item in allCategoriesProd1)
                {
                    model.AvailableCategoriesForInstallationIn.Add(new SelectListItem { Text = item.Text, Value = item.Value });
                }
                model.AvailableCategoriesForInstallationIn.Insert(0, new SelectListItem { Text = "Select", Value = "0" });

                model.InstructionFileName = product.InstructionFileName;
                model.InstructionFilePath = product.InstructionFilePath;
                model.IsInstructionFileModel = product.IsInstructionFile;
            ...
            Product product, ProductAttributeCombination productAttributeCombination, bool excludeProperties = false)
        {
            ...
            if (productAttributeCombination != null)
            {
                ...
                model ??= new ProductAttributeCombinationModel
                {
                    ...
                    AttBinLocation = productAttributeCombination.AttBinLocation
                };
            }
