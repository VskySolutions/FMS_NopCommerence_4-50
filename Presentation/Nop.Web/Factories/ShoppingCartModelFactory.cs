...
namespace Nop.Web.Factories
{
    ...
    public partial class ShoppingCartModelFactory : IShoppingCartModelFactory
    {
        ...
        ////Added by Yogesh Kumbhar on Dt: 12-13-2024
        ...
        protected virtual async Task<ShoppingCartModel.ShoppingCartItemModel> PrepareShoppingCartItemModelAsync(IList<ShoppingCartItem> cart, ShoppingCartItem sci)
        {
            ...
            var cartItemModel = new ShoppingCartModel.ShoppingCartItemModel
            {
                ...
                IsEstimate = sci.IsEstimate // Added by Yogesh Kumbhar on Dt: 01-17-2025
            };
            ...
            bool prepareAndDisplayOrderReviewData = false)
        {
            ...
            //model.IsEstimateFlag = false; //Added by Yogesh Kumbhar on Dt: 01-21-2025
            //cart items
            foreach (var sci in cart)
            {
                var cartItemModel = await PrepareShoppingCartItemModelAsync(cart, sci);
                ////Added by Yogesh Kumbhar on Dt: 01-21-2025
                //if (cartItemModel.IsEstimate)
                //    model.IsEstimateFlag = true;
            ...
            {
                model.EstimateShipping = await PrepareEstimateShippingModelAsync(cart, setEstimateShippingDefaultAddress);
            }
        ...
        public virtual async Task<OrderTotalsModel> PrepareOrderTotalsModelAsync(IList<ShoppingCartItem> cart, bool isEditable)
        {
            ...
            if (cart.Any())
            {
                ...
                if (subTotalWithoutDiscountBase - orderSubTotalDiscountAmountBase <= 50)
                {
                    model.IsLessSubTotal = true;
                }
                ...
                if ((_taxSettings.HideTaxInOrderSummary && await _workContext.GetTaxDisplayTypeAsync() == TaxDisplayType.IncludingTax) || CustomerRole.IsTaxExempt) // (Mohit :- 10-07-2024 :- Skip if IsTaxExempt.)
                {
                    displayTax = true;
                ...
                {
                    bool IsBMWDealer = customer.CustomerAccType == 6;
                    if ((shoppingCartTotalBase == 0 && _taxSettings.HideZeroTax) || IsBMWDealer || customer.IsTaxExempt) // (//Mohit :- 10-07-2024 :- Skip if BMW Dealer)
                    {
                        displayTax = true;
                        displayTaxRates = false;
                        model.Tax = "$0.00";
                    ...
                    else
                    {
                        ...
                        //if ((bool)IsCheckOut)
                        //{
                        displayTaxRates = _taxSettings.DisplayTaxRates;
                        displayTax = !displayTaxRates;

                        //Mohit :- 10-07-2024 :- Skip if IsTaxExempt or BMW Dealer. 
                        //Custom TaxJar API Code
                        ...
                        if (customer.ShippingAddressId != null && customer.ShippingAddressId > 0)
                        {
                            ...
                            if (address != null && !string.IsNullOrEmpty(address.ZipPostalCode))
                            {
                                ...
                                decimal TotalAfterDiscount = subtotalBase - orderSubTotalDiscountAmountBase;
                                var (IsError, ErrorMessage, OrderTaxAmount, OrderTaxPercentage) = await _taxJarService.GetTaxOnOrder(TaxAddress, TotalAfterDiscount);
                                if (IsError)
                                    await _logger.ErrorAsync(ErrorMessage, null, customer);

                                model.Tax = await _priceFormatter.FormatPriceAsync(OrderTaxAmount, true, false);
                                model.TaxRates.Add(new OrderTotalsModel.TaxRate
                                {
                                    Rate = OrderTaxPercentage.ToString(),
                                    Value = OrderTaxAmount.ToString(),
                                });
                            }
                            else
                            {
