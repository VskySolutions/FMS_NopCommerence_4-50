...
namespace Nop.Web.Factories
{
    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
        ...
        public virtual async Task<OrderDetailsModel> PrepareOrderDetailsModelAsync(Order order)
        {
            ...
            model.IsLessSubTotal = (bool)order.IsLessSubTotal;
            ...
            if (order.CustomerTaxDisplayType == TaxDisplayType.IncludingTax && !_taxSettings.ForceTaxExclusionFromOrderSubtotal)
            {
                ...
                var primaryStoreCurrency = _currencyService.GetCurrencyByIdAsync(_currencySettings.PrimaryStoreCurrencyId);
                if (primaryStoreCurrency == null)
                    throw new Exception("Cannot load primary store currency");

                if (Convert.ToBoolean(order.IsLessSubTotal))
                {
                    model.WithoutHandlingAmt = await _priceFormatter.FormatPriceAsync((order.OrderSubtotalInclTax - 10), true, order.CustomerCurrencyCode, languageId, false);
                }
            }
            else
            {
            ...
            if ((_taxSettings.HideTaxInOrderSummary && order.CustomerTaxDisplayType == TaxDisplayType.IncludingTax) || customer.IsTaxExempt) //(Mohit :- 10-07-2024 :- Skip if IsTaxExempt)
            {
                displayTax = true;
            ...
            else
            {
                ...
                if ((order.OrderTax == 0 && _taxSettings.HideZeroTax) || IsBMWDealer || customer.IsTaxExempt)
                {
                    displayTax = true;
                    displayTaxRates = false;
                    model.Tax = "$0.00";
                ...
                else
                {
                    ...
                    if (customer.ShippingAddressId != null && customer.ShippingAddressId > 0)
                    {
                        ...
                        if (address != null && !string.IsNullOrEmpty(address.ZipPostalCode))
                        {
                            ...
                            decimal TotalAfterDiscount = order.OrderSubtotalExclTax - order.OrderSubTotalDiscountExclTax;
