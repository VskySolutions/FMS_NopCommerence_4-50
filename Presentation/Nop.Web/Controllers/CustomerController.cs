...
namespace Nop.Web.Controllers
{
    ...
    public partial class CustomerController : BasePublicController
    {
        ...
        public virtual async Task<IActionResult> Logout()
        {
            ...
            HttpContext.Session.Remove("ProductSearchFilters");
            return RedirectToRoute("Homepage");
        }
        ...
        public virtual async Task<IActionResult> Register(string returnUrl)
        {
            ...
            //Available Customer Type
            var RolesAailable = _customerService.ALLCustomerRoles();
            for (int i = 0; i < RolesAailable.Count(); i++)
            {
                if (i > 3)
                {
                    model.AvailableCustomerAccType.Add(new SelectListItem { Text = RolesAailable[i].Name, Value = RolesAailable[i].Id.ToString() });
                }
            }
            model.AvailableCustomerAccType.Insert(0, new SelectListItem { Text = "Select", Value = "0" });
            return View(model);
        }

        [HttpPost]
        [IgnoreAntiforgeryToken]
        public JsonResult CheckBMWNumberExistOrNot(int CustId = 0)
        {
            if (CustId > 0)
            {
                var Data = _customerService.GetCustomerByBMwDealerCode(CustId);
        ...
        //available even when navigation is not allowed
        [CheckAccessPublicStore(true)]
        public virtual async Task<IActionResult> Register(RegisterModel model, string returnUrl, bool captchaValid, IFormCollection form, IFormFile formFile)
        {
            //check whether registration is allowed
            if (_customerSettings.UserRegistrationType == UserRegistrationType.Disabled)
                return RedirectToRoute("RegisterResult", new { resultId = (int)UserRegistrationType.Disabled, returnUrl });
            ...
            var CompanyName = model.Company != null ? model.Company.ToLower() : null;
            ...
            if (ModelState.IsValid)
            {
                ...
                //Title DiscriptionSelect from cust
                customer.TitleDiscription = model.TitleDiscription;

                //BMWDealer
                if (model.CucAccType == 6)
                {
                    customer.BMWDealer = true;
                    customer.CustomerNumber = model.CustomerNumber;
                }

                //BMWDealer
                customer.CustomerAccType = model.CucAccType;
                ...
                if (registrationResult.Success)
                {
                    ...
                    if (EmaiFlag)
                    {
                        if (model.Email != null)
                        {
                            //Sent Email to Admin when Tax file is uploaded.
                            await SendEmailToAdmin(model);
                        }
                    }

                    //Sent Email to User
                    await SendEmailToUser(model);
            ...
            for (int i = 0; i < RolesAailable.Count(); i++)
            {
                if (i > 3)
                {
                    model.AvailableCustomerAccType.Add(new SelectListItem
                    {
                        Text = RolesAailable[i].Name,
                        Value = RolesAailable[i].Id.ToString()
                    });
                }
            }
            model.AvailableCustomerAccType.Insert(0, new SelectListItem { Text = "Select", Value = "0" });
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<bool> SendEmailToAdmin(RegisterModel model)
        {
            bool EmailStatus = false;
            System.Net.Mail.MailMessage mail = new System.Net.Mail.MailMessage();
            mail.To.Add("info@fmsaccessories.com");

            mail.Subject = "Tax exempt: " + model.FirstName + " " + model.LastName + " ";
            mail.Body = "\r\n New customer is registered and uploaded their tax exempt details,<br/>\r\n \r\n Customer Name :" + model.FirstName + " " + model.LastName + "  ,<br/> you can verify on : <a href='devfms.vskywebapps.com/login' >FMS solutions,LLC</a> ";
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<bool> SendEmailToUser(RegisterModel model)
        {
            bool EmailStatus = false;
            System.Net.Mail.MailMessage mail = new System.Net.Mail.MailMessage();
            mail.To.Add(model.Email);

            mail.Subject = "Customer is registered successfully.";
            mail.Body = "\r\n You have susscessfully registed. ";
            mail.IsBodyHtml = true;
            await _customerService.SendEmailAsyncEvents(mail);
            return EmailStatus;
        }
        ...
        [HttpPost]
        public virtual async Task<IActionResult> Info(CustomerInfoModel model, IFormCollection form, IFormFile formFile)
        {
            var customer = await _workContext.GetCurrentCustomerAsync();
            if (!await _customerService.IsRegisteredAsync(customer))
                return Challenge();
        ...
        #region My Account / Customer Secondary Emails
        //Added by Yogesh Kumbhar on Dt: 02-14-2025
        public virtual async Task<IActionResult> SecondaryEmails()
        {
            var customer = await _workContext.GetCurrentCustomerAsync();
            if (!await _customerService.IsRegisteredAsync(customer))
                return Challenge();
            ...
            var customerEmails = await _customerService.GetAllCustomerEmailByIdAsync(customer.Id);
            model.CustomerEmailList = customerEmails.Select(email => new CustomerEmailModel
            {
                Id = email.Id,
                CustomerId = email.CustomerId,
                Email = email.Email,
                IsOrder = email.IsOrder,
                IsOrderCancel = email.IsOrderCancel,
                IsOrderShipped = email.IsOrderShipped,
                IsEstimate = email.IsEstimate,
                IsRefund = email.IsRefund,
                Deleted = email.Deleted,
                CreatedOnUtc = email.CreatedOnUtc
            }).ToList();
        ...
        [HttpPost]
        public virtual async Task<IActionResult> CreateAndEditCustmerEmail(CustomerEmailModel model)
        {
            var customer = await _workContext.GetCurrentCustomerAsync();
            if (!await _customerService.IsRegisteredAsync(customer))
                return Challenge();
            ...
            try
            {
                if (string.IsNullOrWhiteSpace(model.Email))
                    throw new NopException("Customer email is Required");

                if (await _customerService.ExistCustomerEmailAsync(model.Email, customer.Id, model.Id))
                    throw new NopException("This email has already been added. Please try a different one.");

                if (model.Id == 0) {
                   
                    var custEmail = new CustomerEmail();
                    custEmail.CustomerId = customer.Id;
                    custEmail.Email = model.Email;
                    custEmail.IsOrder = model.IsOrder;
                    custEmail.IsOrderCancel = model.IsOrderCancel;
                    custEmail.IsOrderShipped = model.IsOrderShipped;
                    custEmail.IsEstimate = model.IsEstimate;
                    custEmail.IsRefund = model.IsRefund;
                    custEmail.CreatedOnUtc = DateTime.UtcNow;
                    await _customerService.InsertCustomerEmailAsync(custEmail);
                } 
                else 
                {
                    CustomerEmail customerEmail = await _customerService.GetCustomerEmailByIdAsync(model.Id);

                    customerEmail.Email = model.Email;
                    customerEmail.IsOrder = model.IsOrder;
                    customerEmail.IsOrderCancel = model.IsOrderCancel;
                    customerEmail.IsOrderShipped = model.IsOrderShipped;
                    customerEmail.IsEstimate = model.IsEstimate;
                    customerEmail.IsRefund = model.IsRefund;
            ...
            return RedirectToRoute("CustomerSecondaryEmails");
        }
        public virtual async Task<IActionResult> EditSecondaryEmail(int emailId)
        {
            var customer = await _workContext.GetCurrentCustomerAsync();
            if (!await _customerService.IsRegisteredAsync(customer))
                return Challenge();
            ...
            model.Email = customerEmail.Email;
            model.IsOrder = customerEmail.IsOrder;
            model.IsOrderCancel = customerEmail.IsOrderCancel;
            model.IsOrderShipped = customerEmail.IsOrderShipped;
            model.IsEstimate = customerEmail.IsEstimate;
            model.IsRefund = customerEmail.IsRefund;
            model.Id = customerEmail.Id;

            return View("SecondaryEmails", model);
        }
        
        [HttpPost]
        [IgnoreAntiforgeryToken]
        public virtual async Task<IActionResult> DeleteCustomerEmail(int emailId)
        {
            var customer = await _workContext.GetCurrentCustomerAsync();
            if (!await _customerService.IsRegisteredAsync(customer))
                return Challenge();
            ...
            try
            {
                await _customerService.DeleteCustomerEmailAsync(customer.Id, emailId);
