...
namespace Nop.Web.Controllers
{
    ...
    public partial class CheckoutController : BasePublicController
    {
        ...
        public virtual async Task<IActionResult> Completed(int? orderId)
        {
            ...
            //Added by Yogesh Kumbhar on Dt: 02-19-2025
            var emailModel = await _orderModelFactory.PrepareOrderDetailsModelAsync(order);

            var ccEmails = await _customerService.GetAllCustomerEmailByFlagsAsync(customer.Id, isOrderPlaced: true);
            ...
            mail.Subject = "Order receipt from FMS Solutions, LLC.";

            StringBuilder emailBody = new StringBuilder();
            emailBody.Append($@"
                                <p>Hello {emailModel.BillingAddress.FirstName} {emailModel.BillingAddress.LastName},<br />
                                Thanks for buying from FMS Solutions, LLC. Below is the summary of the order.</p>
                                <p>Order Number: {emailModel.Id}<br/>
                                Order Details: <a href='{baseUrl}{emailModel.Id}' target='_blank'> {baseUrl}{emailModel.Id}</a><br />
                                Date Ordered: {emailModel.CreatedOn}</p><br />
                                <p><strong>Billing Address:</strong><br />
                                {emailModel.BillingAddress.FirstName} {emailModel.BillingAddress.LastName},<br />
                                {emailModel.BillingAddress.Address1},<br />
                                {emailModel.BillingAddress.City}, {emailModel.BillingAddress.ZipPostalCode}<br />
                                {emailModel.BillingAddress.StateProvinceName}, {emailModel.BillingAddress.CountryName}</p>
            
                                <p><strong>Shipping Address:</strong><br />
                                {emailModel.ShippingAddress.FirstName} {emailModel.ShippingAddress.LastName},<br />
                                {emailModel.ShippingAddress.Address1},<br />
                                {emailModel.ShippingAddress.City}, {emailModel.ShippingAddress.ZipPostalCode}<br />
                                {emailModel.ShippingAddress.StateProvinceName}, {emailModel.ShippingAddress.CountryName}</p>
            
                                <p>Shipping Method: {emailModel.ShippingMethod}<br />
                                Payment Method: {emailModel.PaymentMethod}</p>
            ...
            foreach (var item in emailModel.Items)
            {
                emailBody.Append($@"
                ...
                emailBody.Append($@"<br /><strong>SKU:</strong> {item.Sku}
                                        ...
                                        <td style='padding: 0.6em 0.4em;text-align: right;'>{item.UnitPriceValue:C}</td>
                                        <td style='padding: 0.6em 0.4em;text-align: center;'>{item.Quantity}</td>
                                        <td style='padding: 0.6em 0.4em;text-align: right;'>{item.SubTotalValue:C}</td>
                                    </tr>");
            }

            emailBody.Append($@"<tr style=""text-align:right;""><td>&nbsp;</td><td colspan=""2"" style=""background-color: #dde2e6;padding:0.6em 0.4em;""><strong>Sub-Total:</strong></td> <td style=""background-color: #dde2e6;padding:0.6em 0.4em;""><strong>{emailModel.OrderSubtotalValue:C}</strong></td></tr>");
            if (emailModel.OrderSubTotalDiscountValue > 0)
            {
                emailBody.Append($@" <tr style=""text-align:right;"" ><td>&nbsp;</td><td colspan=""2"" style=""background-color: #dde2e6;padding:0.6em 0.4em;""><strong>Discount:</strong></td> <td style=""background-color: #dde2e6;padding:0.6em 0.4em;""><strong>{emailModel.OrderSubTotalDiscountValue:C}</strong></td></tr>");
            }
            emailBody.Append($@"<tr style=""text-align:right;""><td>&nbsp;</td><td colspan=""2"" style=""background-color: #dde2e6;padding:0.6em 0.4em;""><strong>Shipping:</strong></td> <td style=""background-color: #dde2e6;padding:0.6em 0.4em;""><strong>{emailModel.OrderShipping}</strong></td></tr>
                                    <tr style=""text-align:right;""><td>&nbsp;</td><td colspan=""2"" style=""background-color: #dde2e6;padding:0.6em 0.4em;""><strong>Tax:</strong></td> <td style=""background-color: #dde2e6;padding:0.6em 0.4em;""><strong>{emailModel.Tax}</strong></td></tr>");
            if (emailModel.IsLessSubTotal)
            {
            ...
            emailBody.Append($@"<tr style=""text-align:right;""><td>&nbsp;</td><td colspan=""2"" style=""background-color: #dde2e6;padding:0.6em 0.4em;""><strong>Order Total:</strong></td> <td style=""background-color: #dde2e6;padding:0.6em 0.4em;""><strong>{emailModel.OrderTotal}</strong></td></tr>
            </table>");
        ...
        protected virtual async Task<JsonResult> OpcLoadStepAfterShippingMethod(IList<ShoppingCartItem> cart)
        {
            ...
            if (isPaymentWorkflowRequired)
            {
                ...
                if (_workContext.GetCurrentCustomerAsync().Result.IsOpenPayment)
                {
                    paymentMethodModel.PaymentMethods.Add(new CheckoutPaymentMethodModel.PaymentMethodModel()
                    {
                        Name = "Open Payment",
                        Description = "You can make a payment within a month",
                        PaymentMethodSystemName = "Open Payment",
                        LogoUrl = "1"
                    });
                }
                if (_paymentSettings.BypassPaymentMethodSelectionIfOnlyOne &&
        ...
        {
            //Open Payment
            if (_workContext.GetCurrentCustomerAsync().Result.IsOpenPayment && paymentMethod == null)
            {

            }
            else if (paymentMethod.SkipPaymentInfo ||
                (paymentMethod.PaymentMethodType == PaymentMethodType.Redirection && _paymentSettings.SkipPaymentInfoStepForRedirectionPaymentMethods))
            {
                //skip payment info page
                var paymentInfo = new ProcessPaymentRequest();
            ...
            if (_workContext.GetCurrentCustomerAsync().Result.IsOpenPayment && paymentMethod == null)
            {
                //var paymenInfoModel = _checkoutModelFactory.PreparePaymentInfoModelAsync(paymentMethod);
                //CheckoutPaymentInfoModel obj = new CheckoutPaymentInfoModel();
                ...
                return Json(new
                {
                    ...
                    {
                        name = "payment-info",
                        html = await RenderPartialViewToStringAsync("_OpenPayment", null)
            ...
            try
            {
                ...
                if (_shippingSettings.AllowPickupInStore && _orderSettings.DisplayPickupInStoreOnShippingMethodPage)
                {
                    ...
                    await _genericAttributeService.SaveAttributeAsync<PickupPoint>(
                        customer, NopCustomerDefaults.SelectedPickupPointAttribute, null, store.Id);
                }
                ...
                if (shippingOptions == null || !shippingOptions.Any())
                {
                    shippingOptions = (await _shippingService.GetShippingOptionsAsync(
                        cart, await _customerService.GetCustomerShippingAddressAsync(customer),
                        customer, shippingRateComputationMethodSystemName, store.Id)).ShippingOptions.ToList();
                }
                else
                {
                    shippingOptions = shippingOptions
                        .Where(so => so.ShippingRateComputationMethodSystemName.Equals(
                ...
                var usCountry = await _countryService.GetCountryByTwoLetterIsoCodeAsync("US");

                if (shippingAddress != null && usCountry != null)
                {
                    var isUSAddress = shippingAddress.CountryId == usCountry.Id;

                    if (isUSAddress)
                    {
                        // US Customers: show Free Shipping + Premium options
                        var usOptions = shippingOptions
                            .Where(o =>
                                o.Name.Contains("Free Shipping", StringComparison.InvariantCultureIgnoreCase) ||
                                o.Name.Contains("Overnight", StringComparison.InvariantCultureIgnoreCase) ||
                                o.Name.Contains("2Day", StringComparison.InvariantCultureIgnoreCase) ||
                                o.Name.Contains("Next Day", StringComparison.InvariantCultureIgnoreCase))
                            .OrderBy(o => o.Name.Contains("Free Shipping", StringComparison.InvariantCultureIgnoreCase) ? 0 : 1)
                            .ThenBy(o => o.Rate)
                            .ToList();

                        if (usOptions.Any())
                        {
                            shippingOptions = usOptions;
                            await _logger.InformationAsync("US Address detected — showing Free + Premium options.");
                        }
                    }
                    else
                    {
                        // International Customers: only FedEx
                        var intlOptions = shippingOptions
                            .Where(o => o.ShippingRateComputationMethodSystemName.Contains("FedEx", StringComparison.InvariantCultureIgnoreCase) ||
                                        o.Name.Contains("FedEx", StringComparison.InvariantCultureIgnoreCase))
                            .ToList();

                        if (intlOptions.Any())
                        {
                            shippingOptions = intlOptions;
                            await _logger.InformationAsync("International Address detected — showing only FedEx options.");
                        }
                    }
                }
                ...
                // ✅ SAVE SELECTION (nopCommerce standard logic)
                await _genericAttributeService.SaveAttributeAsync(
                    customer, NopCustomerDefaults.SelectedShippingOptionAttribute, shippingOption, store.Id);

                // ✅ Continue to next checkout step (price calculation preserved)
                return await OpcLoadStepAfterShippingMethod(cart);
            }
            catch (Exception exc)
            {
        ...
        public virtual async Task<IActionResult> OpcSavePaymentInfo(IFormCollection form)
        {
            try
            {
                ...
                var paymentMethod = await _paymentPluginManager
                    .LoadPluginBySystemNameAsync(paymentMethodSystemName, customer, store.Id);
                if (paymentMethodSystemName == "Open Payment" && paymentMethod == null)
                {
                ...
                if (paymentMethod != null && paymentMethodSystemName != "Open Payment")
                {
                    var warnings = await paymentMethod.ValidatePaymentFormAsync(form);
                    foreach (var warning in warnings)
                        ModelState.AddModelError("", warning);
                ...
                else
                {
                    ...
                    //session save
                    //_httpContext.Session["OrderPaymentInfo"] = obj;
                    HttpContext.Session.Set<ProcessPaymentRequest>("OrderPaymentInfo", ProcessPaymentRequest);
                    var confirmOrderModel = await _checkoutModelFactory.PrepareConfirmOrderModelAsync(cart);
                    return Json(new
                    {
        ...
        public virtual async Task<IActionResult> OpcConfirmOrder()
        {
            try
            {
                ...
                if (_workContext.GetCurrentCustomerAsync().Result.IsOpenPayment && processPaymentRequest == null)
                {
                    ProcessPaymentRequest obj = new ProcessPaymentRequest();
                    obj.StoreId = _storeContext.GetCurrentStoreAsync().Result.Id;
                }
                else if (processPaymentRequest == null)
                {
                    //Check whether payment workflow is required
                    if (await _orderProcessingService.IsPaymentWorkflowRequiredAsync(cart))
                    {
