...
namespace Nop.Web.Controllers
{
    ...
    public partial class CatalogController : BasePublicController
    {
        ...
        private readonly IHomePageArrivals _homepagearrivals;
        ...
        private readonly IMarquee _marquee;
        #endregion

        #region Ctor

        public CatalogController(CatalogSettings catalogSettings,
            IHomePageArrivals homepagearrivals,
            IMarquee marquee,
            ...
            VendorSettings vendorSettings)
        {
            ...
            _marquee = marquee;
            _homepagearrivals = homepagearrivals;
        ...
        /// </summary>
        /// <returns></returns>
        public virtual ActionResult DynamicSlider()
        {
            var mylistModel = _homepagearrivals.HomePageList();
            var model = new TopMenuModel();
            var myliost = new List<TopMenuModel>();
            foreach (var item in mylistModel)
            {
                var obj = new HomePageArrivalModel();
                obj.ProductName = item.ProductName;
                obj.ImagePath = item.ImagePath;
                model.NewSlider.Add(obj);
            }
        ...
        public virtual ActionResult MarqueeText()
        {
            var model = _marquee.GeMarqueeById(1);
            var webmodel = new MarqueeModelWeb();
            webmodel.Id = model.Id;
            //webmodel.Text = model.Text;
            return PartialView(webmodel);
        }
        ...
        public async Task<IActionResult> InstallationFile(int id)
        {
            string MyStr = string.Empty;
            var model = new CategoryModel();
            var list = _productService.GetProductsByCateId(id);
            foreach (var item in list)
            {
                InstallationModel obj = new InstallationModel();
                obj.ProdName = item.Name;
                obj.InstructonFileName = item.InstructionFileName;
                obj.InstructionFilePath = item.InstructionFilePath;

                var ProductInfo = await _productService.GetProductPicturesByProductIdAsync(item.Id);
                int pictureSize = _mediaSettings.CategoryThumbPictureSize;
                foreach (var pic in ProductInfo)
                {
                    var picture = await _pictureService.GetPictureByIdAsync(pic.PictureId);
                    obj.ProductImagePath = (await _pictureService.GetPictureUrlAsync(picture, 100)).Url;
                    break;
                }
                model.InstallationInstructionFiles.Add(obj);
            }
            return PartialView("_InstallationInstruction", model);
        }
        #endregion
    }
}