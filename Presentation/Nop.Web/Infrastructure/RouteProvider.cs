...
namespace Nop.Web.Infrastructure
{
    ...
    public partial class RouteProvider : BaseRouteProvider, IRouteProvider
    {
        ...
        public void RegisterRoutes(IEndpointRouteBuilder endpointRouteBuilder)
        {
            ...
            endpointRouteBuilder.MapControllerRoute(name: "VideoPGallery",
                pattern: $"VideoPGallery",
                defaults: new { controller = "Home", action = "VideoGalleryMap" });

            //Added by Yogesh Kumbhar on Dt: 12/02/2024
            endpointRouteBuilder.MapControllerRoute(name: "EstimateList",
                pattern: "Estimate/List",
                defaults: new { controller = "Estimate", action = "EstimateList" });

            //customer account links
            endpointRouteBuilder.MapControllerRoute(name: "NewEstimate", 
                pattern: "New/Estimate",
                defaults: new { controller = "Estimate", action = "AddList" });

            //customer account links
            endpointRouteBuilder.MapControllerRoute(name: "AddCopyEstimate",
                pattern: "New/Product",
                defaults: new { controller = "Product", action = "CopyEstimate" });

            //comparing products
            endpointRouteBuilder.MapControllerRoute(name: "goestimate",
                pattern: "myEstimate/{EstimateId}",
                defaults: new { controller = "Estimate", action = "ShowEstimateShop" },
                                new { EstimateId = @"\d+" },
                                new[] { "Nop.Web.Controllers" }
                                 );


            // Update Estimate Item's
            endpointRouteBuilder.MapControllerRoute(name: "Estimate",
                pattern: "estimate/",
                defaults: new { controller = "Estimate", action = "ShowEstimateShop" });
            ...
            endpointRouteBuilder.MapControllerRoute(name: "CustomerSecondaryEmails",
                pattern: $"{lang}/customer/secondaryemails",
                defaults: new { controller = "Customer", action = "SecondaryEmails" });

            //customer account links
            //Added by Yogesh Kumbhar on Dt: 02-15-2025
            endpointRouteBuilder.MapControllerRoute(name: "gocustomeremail",
                pattern: "customer/secondaryemail/{emailId}",
                defaults: new { controller = "Customer", action = "EditSecondaryEmail" },
                                new { emailId = @"\d+" },
                                new[] { "Nop.Web.Controllers" }
                                 );
