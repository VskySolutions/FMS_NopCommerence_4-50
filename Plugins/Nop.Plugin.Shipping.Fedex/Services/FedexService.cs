...
namespace Nop.Plugin.Shipping.Fedex.Services
{
    public class FedexService
    {
        ...
        private async Task<IList<ShippingOption>> ParseResponseAsync(RateReply reply, Currency requestedShipmentCurrency)
        {
            ...
            {
                // Added by Sumesh Patil on Dt: 01-23-2026
                // Remove FEDEX_ prefix for international services as our version of ServiceType dosent contains FEDEX_
                if (rateDetail.ServiceType.Contains("INTERNATIONAL"))
                {
                    rateDetail.ServiceType = rateDetail.ServiceType.Replace("FEDEX_", "");
                }
