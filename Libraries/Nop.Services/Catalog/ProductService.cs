...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        /// </returns>
        protected virtual async Task<(string sku, string manufacturerPartNumber, string gtin, string bin)> GetSkuMpnGtinAsync(Product product, string attributesXml)
        {
            if (product == null)
                ...
                product.ManageInventoryMethod == ManageInventoryMethod.ManageStockByAttributes)
            {
                ...
                if (combination != null)
                {
                    ...
                    bin = combination.AttBinLocation;
                }
            }
            ...
            return (sku, manufacturerPartNumber, gtin, bin);
        }
        ...
        /// The task result contains the products
        /// </returns>
        public virtual async Task<IPagedList<Product>> ProductSearchAsync(
            int pageIndex = 0,
            int pageSize = int.MaxValue,
            IList<int> categoryIds = null,
            IList<int> manufacturerIds = null,
            ...
            bool? overridePublished = null)
        {
            ...
            if (IsInstructionFile==true)
                productsQuery = productsQuery.Where(p => !p.Deleted && p.IsInstructionFile);
        ...
        public virtual async Task<string> FormatSkuAsync(Product product, string attributesXml = null)
        {
            ...
            var (sku, _, _,_) = await GetSkuMpnGtinAsync(product, attributesXml);

            return sku;
        }
        public virtual async Task<string> FormatBinlocationAsync(Product product, string attributesXml = null)
        {
            if (product == null)
                throw new ArgumentNullException(nameof(product));

            var (_, _, _, bin) = await GetSkuMpnGtinAsync(product, attributesXml);
        ...
        public virtual async Task<string> FormatMpnAsync(Product product, string attributesXml = null)
        {
            ...
            var (_, manufacturerPartNumber, _,_) = await GetSkuMpnGtinAsync(product, attributesXml);
        ...
        public virtual async Task<string> FormatGtinAsync(Product product, string attributesXml = null)
        {
            ...
            var (_, _, gtin,_) = await GetSkuMpnGtinAsync(product, attributesXml);
        ...
        public virtual IList<Product> GetProductsByCateId(int CateId)
        {
            var query = from p in _productRepository.Table
                        ...
                        !p.Deleted
                        select p;
            query = query.Where(a => a.ProdCateId == CateId);
