...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        //Added by Yogesh Kumbhar on Dt: 01-15-2025 & 01-25-2025
        ...
        public virtual async Task<IPagedList<Customer>> GetAllCustomersAsync(DateTime? createdFromUtc = null, DateTime? createdToUtc = null,
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false, int CustomerNumber = 0)
        {
        ...
        /// <summary>
        /// Send Email AsyncEvents
        /// </summary>
        /// <param name="mail"></param>
        /// <returns></returns>
        /// <exception cref="ApplicationException"></exception>
        public async Task SendEmailAsyncEvents(System.Net.Mail.MailMessage mail)
        {
            try
            {
                var emailAccount = _emailAccountRepository.Table.FirstOrDefault(); // Yogesh Kumbhar on Dt: 01-15-2025
                await Task.Run(async () =>
                {
                    // Added SMTP Server details by Yogesh Kumbhar on Dt: 01-13-2025 & 01-15-2025
                    SmtpClient smtp = new SmtpClient();
                    smtp.Port = emailAccount.Port;
                    smtp.Host = emailAccount.Host;
                    smtp.UseDefaultCredentials = false;
                    smtp.EnableSsl = true;
                    smtp.Credentials = new System.Net.NetworkCredential(emailAccount.Username, emailAccount.Password);
        ...
        /// <summary>
        /// Get Customer ByBMw Dealer Code
        /// </summary>
        /// <param name="DealerCode"></param>
        /// <returns></returns>
        public virtual IList<Customer> GetCustomerByBMwDealerCode(int DealerCode)
        {
            if (DealerCode == 0)
                return null;
            ...
            query = query.Where(n => n.CustomerNumber == DealerCode);
        ...
        public virtual async Task DeleteCustomerAsync(Customer customer)
        {
            ...
            if (!String.IsNullOrEmpty(customer.QBOID))
                customer.IsRecentlyDeleted = true;
        ...
        public virtual async Task<Customer> GetCustomerByEmailAsync(string email)
        {
            ...
            var query = from c in _customerRepository.Table
                        orderby c.Id
                        where c.Email == email && c.Deleted != true
                        select c;
        ...
        public virtual IList<Customer> GetDelearCodeExistCount(int DelearCode = 0, int Custid = 0)
        {
            if (DelearCode == 0 || Custid == 0)
                return null;
            var query = _customerRepository.Table;
            query = query.Where(x => x.BMWDealer == true && x.Deleted != true);
            query = query.Where(x => x.Id != Custid && x.CustomerNumber == DelearCode);
            return query.ToList();
        }
        ...
        public virtual IList<CustomerRole> ALLCustomerRoles()
        {
            var query = from cr in _customerRoleRepository.Table
                        select cr;
        ...
        public virtual List<Customer> GetCustomersForSyncingToTaxJarList(List<int> CustomerIds)
        {
            var query = _customerRepository.Table.AsQueryable();

            //Get Registered Role Customers
            int[] customerRoleIds = new int[1];
            customerRoleIds[0] = 3;
            query = query.Join(_customerCustomerRoleMappingRepository.Table, x => x.Id, y => y.CustomerId, (x, y) => new { Customer = x, Mapping = y })
                    .Where(z => customerRoleIds.Contains(z.Mapping.CustomerRoleId))
                    .Select(z => z.Customer)
                    .Distinct();
            ...
            {
                if (CustomerIds.Count() == 1 && CustomerIds[0] == 0)
                    return query.ToList();
                else
                    return query.Where(m => CustomerIds.Contains(m.Id)).ToList();
            }

            return null;
        }

        public virtual int[] GetCustomerIdsForTaxJar(bool? IsSyncedToTaxJar, bool? IsRecentlyModified, bool? IsRecentlyDeleted, bool? IsDeleted)
        {
            var query = _customerRepository.Table.Where(m => m.Email != null && m.Email != "").AsQueryable();

            //Get Registered Role Customers
            int[] customerRoleIds = new int[1];
            customerRoleIds[0] = 3;
            query = query.Join(_customerCustomerRoleMappingRepository.Table, x => x.Id, y => y.CustomerId, (x, y) => new { Customer = x, Mapping = y }).Where(z => customerRoleIds.Contains(z.Mapping.CustomerRoleId)).Select(z => z.Customer).Distinct();

            if (IsSyncedToTaxJar != null)
            {
                if ((bool)IsSyncedToTaxJar)
                    query = query.Where(m => m.IsSyncedToTaxJar);
                else
                    query = query.Where(m => !m.IsSyncedToTaxJar);
            }

            if (IsRecentlyModified != null)
            {
                if ((bool)IsRecentlyModified)
                    query = query.Where(m => m.IsRecentlyModified);
                else
                    query = query.Where(m => !m.IsRecentlyModified);
            }

            if (IsRecentlyDeleted != null)
            {
                if ((bool)IsRecentlyDeleted)
                    query = query.Where(m => m.IsRecentlyDeleted);
                else
                    query = query.Where(m => !m.IsRecentlyDeleted);
            }
        ...
        //Added by Yogesh Kumbhar on Dt: 01-15-2025 & 01-16-2025
        public async Task<List<SelectListItem>> GetCustomerSelectListAsync()
        {
            var query = from c in _customerRepository.Table
                        join fn in _gaRepository.Table
                            on new { EntityId = c.Id, KeyGroup = "Customer", Key = "FirstName" }
                            equals new { fn.EntityId, fn.KeyGroup, fn.Key }
                        join ln in _gaRepository.Table
                            on new { EntityId = c.Id, KeyGroup = "Customer", Key = "LastName" }
                            equals new { ln.EntityId, ln.KeyGroup, ln.Key }
                        join cm in _gaRepository.Table
                            on new { EntityId = c.Id, KeyGroup = "Customer", Key = "Company" }
                            equals new { cm.EntityId, cm.KeyGroup, cm.Key } into cmGroup
                        from cm in cmGroup.DefaultIfEmpty()
                        join ca in (from ca in _customerAddressMappingRepository.Table
                                    group ca by ca.CustomerId into g
                                    select new
                                    {
                                        CustomerId = g.Key,
                                        MinAddressId = g.Min(x => x.AddressId)
                                    }) on c.Id equals ca.CustomerId into caGroup
                        from ca in caGroup.DefaultIfEmpty()
                        join a in _customerAddressRepository.Table
                            on ca.MinAddressId equals a.Id into aGroup
                        from a in aGroup.DefaultIfEmpty()
                        where !c.Deleted && (c.Email != null || a.Email != null)
                        orderby c.Id
                        select new SelectListItem
                        {
                            Value = c.Id.ToString(),
                            Text = string.Format("{0} {1}",
                                   (c.Email ?? a.Email).ToLower(),
                                   !string.IsNullOrEmpty(cm.Value) || !string.IsNullOrEmpty(a.Company) ? $"({cm.Value ?? a.Company})" : string.Empty
                            ).Trim()
                        };
            ...
            return customerList.OrderBy(x => x.Text).ToList();
        }
        #endregion

        #region Customer Secondary Email Methods
        //Added by Yogesh Kumbhar on Dt: 01-27-2025 To 01-31-2025
        public virtual async Task InsertCustomerEmailAsync(CustomerEmail customerEmail)
        {
            await _customerEmailRepository.InsertAsync(customerEmail);
        }
        public async Task<bool> ExistCustomerEmailAsync(string email, int customerId, int emailId = 0)
        {
            if (string.IsNullOrWhiteSpace(email))
                return false;

            // Check if the email exists in the repository table
            var emailExist = await _customerEmailRepository.Table.AnyAsync(c => c.Email == email && c.CustomerId == customerId && c.Id != emailId && c.Deleted == false);

            return emailExist;
        }
        public virtual async Task<IList<CustomerEmail>> GetAllCustomerEmailByIdAsync(int customerId)
        {
            var query = _customerEmailRepository.Table.Where(x => x.Deleted == false && x.CustomerId == customerId).OrderByDescending(x => x.Id);

            return await query.ToListAsync();
        }
        public virtual async Task DeleteCustomerEmailAsync(int customerId, int emailId)
        {
            if (customerId == 0 && emailId == 0)
                throw new InvalidOperationException("Customer email not found.");

            var custEmail = await _customerEmailRepository.Table
                .FirstOrDefaultAsync(x => x.Id == emailId && x.CustomerId == customerId);

            if (custEmail == null)
                throw new InvalidOperationException("Customer email not found.");

            custEmail.Deleted = true;

            await _customerEmailRepository.UpdateAsync(custEmail, false);
        }
        public virtual async Task UpdateCustomerEmailAsync(CustomerEmail customerEmail)
        {
            await _customerEmailRepository.UpdateAsync(customerEmail);
        }
        public virtual async Task<CustomerEmail> GetCustomerEmailByIdAsync(int emailId)
        {
            if (emailId == 0)
                return null;

            return await _customerEmailRepository.GetByIdAsync(emailId);
        }
        public virtual async Task<IList<string>> GetAllCustomerEmailByFlagsAsync(int customerId, bool isOrderPlaced = false, bool isOrderCancel = false, bool isEstimate = false, bool isOrderRefund = false, bool isOrderShipped = false)
        {
            if (customerId <= 0 || (!isOrderPlaced && !isOrderCancel && !isEstimate && !isOrderRefund && !isOrderShipped))
                return new List<string>();

            var query = _customerEmailRepository.Table
                .Where(x => !x.Deleted && x.CustomerId == customerId);

            // Ensure at least one of the flags is true
            query = query.Where(x => (isOrderPlaced && x.IsOrder) || (isOrderCancel && x.IsOrderCancel) || (isEstimate && x.IsEstimate) || (isOrderRefund && x.IsRefund) || (isOrderShipped && x.IsOrderShipped));

            return await query.Select(x => x.Email).ToListAsync();
        }
        #endregion

        #region Get customer details by Id for Estimate PDF & Excel
        //Added by Yogesh Kumbhar on Dt: 02-11-2025 & 02-12-2025
        public async Task<string> GetCustomerEmailWithCompanyAsync(int customerId)
        {
            var query = from c in _customerRepository.Table
                        join fn in _gaRepository.Table
                            on new { EntityId = c.Id, KeyGroup = "Customer", Key = "FirstName" }
                        ...
                        join cm in _gaRepository.Table
                            on new { EntityId = c.Id, KeyGroup = "Customer", Key = "Company" }
                            equals new { cm.EntityId, cm.KeyGroup, cm.Key } into cmGroup
                        from cm in cmGroup.DefaultIfEmpty() // Handle null cases
                        join ca in (from ca in _customerAddressMappingRepository.Table
                                    group ca by ca.CustomerId into g
                                    select new
                                    {
                        ...
                        select new
                        {
                            FullName = (fn.Value + " " + ln.Value).Trim(),
                            Company = cm != null ? cm.Value : a.Company,
                            Address = (a.Address1 + "\n" + a.City + ", " + a.ZipPostalCode).Trim(),
                            Phone = "C: " + a.PhoneNumber
                        };
            ...
            // Concatenating details with newline separation
            return string.Join("\n",
                new[] { customer.FullName, customer.Company, customer.Address, customer.Phone }
