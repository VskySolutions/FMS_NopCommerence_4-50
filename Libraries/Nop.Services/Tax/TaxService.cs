...
namespace Nop.Services.Tax
{
    ...
    public partial class TaxService : ITaxService
    {
        ...
        protected readonly ITaxJarService _taxJarService;
        #endregion
            ...
            bool priceIncludesTax)
        {
            ...
            //If Customer is tax exempt or BMW dealer then skip i.e return same value
            if(customer.IsTaxExempt || customer.CustomerAccType == 6)
                return (price, taxRate);
            ...
            #region Custom TaxJar API Code
            //Mohit :- 10-09-2024 :- Skip if IsTaxExempt or BMW Dealer. 
            ...
            if (customer.ShippingAddressId != null && customer.ShippingAddressId > 0)
            {
                ...
                //tax exempt
                isTaxable = !await IsTaxExemptAsync(product, customer);

                //make EU VAT exempt validation (the European Union Value Added Tax)
                if (isTaxable && _taxSettings.EuVatEnabled && await IsVatExemptAsync(address, customer))
                    isTaxable = false;
            }
            if (!string.IsNullOrEmpty(TaxAddress.ZipCode))
            {
                (IsError, ErrorMessage, taxRate) = await _taxJarService.GetTaxRateByNexus(TaxAddress);
                if (IsError)
