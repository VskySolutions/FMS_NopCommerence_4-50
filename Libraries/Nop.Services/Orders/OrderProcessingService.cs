...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected class PlaceOrderContainer
        {
            ...
            /// Is Less SubTotal
            /// </summary>
            public bool IsLessSubTotal { get; set; }
        ...
        protected virtual async Task<PlaceOrderContainer> PreparePlaceOrderDetailsAsync(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            //Check If User is a BMW Dealer
            bool IsBMWDealer = _workContext.GetCurrentCustomerAsync().Result.CustomerAccType == 6;
            ...
            ////For admin convert estimate into order
            ////Added  by Yogesh Kumbhar on Dt: 01-07-2025
            //var Customer = await _workContext.GetCurrentCustomerAsync();
            //if (Customer.TitleDiscription != null && Customer.TitleDiscription.Equals("Admin"))
            //{
            //    details.Cart = await _shoppingCartService.GetShoppingCartAsync(Customer, ShoppingCartType.ShoppingCart, processPaymentRequest.StoreId);
            //}
            //else
            //{
            ...
            //Mohit :- 10-07-2024 
            decimal OrderTaxAmount = decimal.Zero;
            decimal OrderTaxPercentage = decimal.Zero;
            //Custom TaxJar API Code -> tax amount (MT)
            //Find Shipping Zipcode.
            if (!IsBMWDealer && !details.Customer.IsTaxExempt)
            {   
                if(details.Customer.ShippingAddressId != null && details.Customer.ShippingAddressId > 0)
                {
                    bool IsError;
                    string ErrorMessage = string.Empty;
                    var address = await _addressService.GetAddressByIdAsync((int)details.Customer.ShippingAddressId);
                    if (address != null && !string.IsNullOrEmpty(address.ZipPostalCode))
                    {
                        var TaxAddress = await GenerateTaxAddress(address);
                        decimal TotalAfterDiscount = details.OrderSubTotalExclTax - details.OrderSubTotalDiscountExclTax;
                        (IsError, ErrorMessage, OrderTaxAmount, OrderTaxPercentage) = await _taxJarService.GetTaxOnOrder(TaxAddress, TotalAfterDiscount);
                        if (IsError)
                            await _logger.ErrorAsync(ErrorMessage, null, details.Customer);

                        details.OrderSubTotalInclTax = details.OrderSubTotalExclTax + OrderTaxAmount;
                        details.OrderTaxTotal = OrderTaxAmount;
                        details.TaxRates = OrderTaxPercentage + ":" + OrderTaxAmount + ";";
                    }
                    else
                    {
                        details.OrderTaxTotal = decimal.Zero;
                        details.TaxRates = "0:0;";
                    }
                }
            }
            else
            ...
            //Mohit Order Handling
            decimal SubTotalAfterTax = details.OrderSubTotalExclTax - details.OrderSubTotalDiscountExclTax;
            if (SubTotalAfterTax <= 50)
                details.IsLessSubTotal = true;

            processPaymentRequest.OrderTotal = details.OrderTotal; //PayPal Payment issue
            ...
            ProcessPaymentResult processPaymentResult, PlaceOrderContainer details)
        {
            var order = new Order
            {
                ...
                IsLessSubTotal = details.IsLessSubTotal
            };
        ...
        protected virtual async Task SetOrderStatusAsync(Order order, OrderStatus os, bool notifyCustomer)
        {
            ...
            // Added By Yogesh
            if (!String.IsNullOrEmpty(order.QBOID) && (int)os == 40)
            ...
            {
                //Oen Payment Note.
                //order.ShippingMethod = (order.ShippingMethod=="Open Payment"? "Open Payment You can make a Payment within a month.": order.ShippingMethod);
                ...
                //Oen Payment Note.
                //order.ShippingMethod = (order.ShippingMethod == "Open Payment You can make a Payment within a month." ? "Open Payment" : order.ShippingMethod);
            }
        ...
        protected virtual async Task ProcessOrderPaidAsync(Order order)
        {
            ...
            if(!order.PaymentMethodSystemName.Contains("Open") && order.PaymentMethodSystemName.Contains("PayPal"))
            {
                var (IsError, ErrorMessage, TaxJarTransaction) = await _taxJarService.GetTransactionById(order.Id.ToString());
                ...
                {
                    var Customer = await _customerService.GetCustomerByIdAsync(order.CustomerId);
                    (IsError, ErrorMessage) = await SendTransactionToTaxJar(order, Customer);
                    if (IsError)
                    {
                        _notificationService.ErrorNotification($"TaxJar Sync Error:- {ErrorMessage}");
                        await _logger.ErrorAsync($"Order ID:- {order.Id} -> Error:- {ErrorMessage}", null, Customer);
                    }
                }
            }
        }
        ...
        public virtual async Task<PlaceOrderResult> PlaceOrderAsync(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            bool IsBMWDealer = false;

            var Customer = _workContext.GetCurrentCustomerAsync().Result;

            ////For admin convert estimate into order
            ////Added by Yogesh Kumbhar on Dt: 01-06-2025
            //if (Customer.TitleDiscription != null && Customer.TitleDiscription.Equals("Admin"))
            //{
            //    var Carts = await _shoppingCartService.GetShoppingCartAsync(Customer);
            //    var singleCustomer = Carts.FirstOrDefault();
            //    Customer = await _customerService.GetCustomerByIdAsync((int)singleCustomer.EstimateCustReference);
            //}

            if (Customer.CustomerAccType == 6)
                IsBMWDealer = true;

            processPaymentRequest.IsBMWDealer = IsBMWDealer;
            ...
            try
            {
                ...
                if (processPaymentRequest.PaymentMethodSystemName != "Open Payment")
                {
                     processPaymentResult = await GetProcessPaymentResultAsync(processPaymentRequest, details);
                ...
                #region open payment
                var curruntcustomer = _workContext.GetCurrentCustomerAsync().Result;
                if (processPaymentRequest.PaymentMethodSystemName == "Open Payment" && curruntcustomer.IsOpenPayment)
                {
                    processPaymentResult = new ProcessPaymentResult { NewPaymentStatus = PaymentStatus.Pending };

                    //Sent notification to client
                    await SendOpenPymentNotification(details.Customer);
                }
                else
                {
                    if (!skipPaymentWorkflow)
                    {
                        ...
                        {
                            //Authorise payment processed two times so added this condition for avoid double payment entry.
                            if (processPaymentResult==null)
                                processPaymentResult = await _paymentService.ProcessPaymentAsync(processPaymentRequest);
                        }
                    }
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    //Send Transaction To TaxJar If is not a open payment
                    if(processPaymentRequest.PaymentMethodSystemName != "Open Payment" && processPaymentRequest.PaymentMethodSystemName != "Payments.PayPalStandard")
                    {
                        var (IsError, ErrorMessage, TaxJarTransaction) = await _taxJarService.GetTransactionById(order.Id.ToString());
                        if (TaxJarTransaction != null && !string.IsNullOrEmpty(TaxJarTransaction.TransactionId))
                        {
        ...
        /// <summary>
        /// Synced Transactions To TaxJar
        /// </summary>
        /// <param name="order">order data</param>
        /// <param name="customer">customer data</param>
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains the error flag & error details
        /// </returns>
        public async Task<(bool, string)> SendTransactionToTaxJar(Order order, Customer customer)
        {
            bool IsError = false;
            string ErrorMessage = string.Empty;
            try
            {
                ...
                var ToAddress = new TaxJarAddress();
                if (order.ShippingAddressId != null)
                {
                    var OrderShippingAddress = await _addressService.GetAddressByIdAsync((int)order.ShippingAddressId);
                    // var CustomerAddress = await _customerService.GetCustomerShippingAddressAsync(customer);
                    if (OrderShippingAddress != null)
                        ToAddress = await GenerateTaxAddress(OrderShippingAddress);
                }
                #endregion

                //Transactions
                var model = new TransactionModel();
                model.TransactionId = order.Id.ToString();
                model.CustomerId = customer.IsSyncedToTaxJar ? customer.Id.ToString() : null;
                model.TransactionDate = order.PaidDateUtc != null ? order.PaidDateUtc.Value.ToString("MM/dd/yyyy hh:mm") : order.CreatedOnUtc.ToString("MM/dd/yyyy hh:mm");
                model.Provider = "api";

                //Shipping Address
                model.ToCountry = ToAddress.Country;
                model.ToZip = ToAddress.ZipCode;
                model.ToState = ToAddress.State;
                model.ToCity = ToAddress.City;
                model.ToStreet = ToAddress.Street;
                
                #region LineItems
                decimal TaxPercentage = decimal.Zero;
                decimal DiscountAmount = decimal.Zero;
                decimal SalesTaxAmount = decimal.Zero;

                //Find cart
                var Cart = await _orderService.GetOrderItemsAsync(order.Id);

                //Find discount
                var TotalDiscount = order.OrderSubTotalDiscountExclTax + order.OrderDiscount;

                //Find Tax
                //Not a BMW Dealar & Non-tax exempt customer
                if (customer.CustomerAccType != 6 && !customer.IsTaxExempt)
                    (IsError, ErrorMessage, TaxPercentage) = await _taxJarService.GetTaxRateByNexus(ToAddress);
                if (IsError)
                    await _logger.ErrorAsync($"Order ID:- {order.Id} -> Error:- {ErrorMessage}", null, customer);

                model.ExemptionType = TaxPercentage > 0 ? "non_exempt" : customer.TaxExemptType;
                ...
                foreach (var CartItem in Cart)
                {
                    ...
                    LineItem.Id = CartItem.Id;
                    LineItem.Quantity = CartItem.Quantity;
                    LineItem.ProductIdentifier = Product.Sku;
                    LineItem.Description = Product.Name;
                    LineItem.UnitPrice = CartItem.UnitPriceExclTax;

                    //Discount
                    if (TotalDiscount > 0)
                    {
                        decimal LineItemSubTotal = LineItem.Quantity * LineItem.UnitPrice;
                        decimal LineItemDiscountPercentage = Math.Round((LineItemSubTotal / order.OrderSubtotalExclTax) * 100, 2);

                        LineItem.Discount = TotalDiscount > 0 ? Math.Round((TotalDiscount * LineItemDiscountPercentage)/100, 2) : 0;
                    ...
                    if (TaxPercentage > 0) 
                    {
                        LineItem.SalesTax = TaxPercentage > 0 ? Math.Round((((LineItem.Quantity * LineItem.UnitPrice) - LineItem.Discount) / 100) * TaxPercentage, 2) : 0;
                        SalesTaxAmount += LineItem.SalesTax;
                    }
                ...
                #endregion
                //Mohit:- 10-19-2024 :- Order handling charge as lineitem
                //Check if subtotal after discount is less than 50. If Yes, Then add order handling charges.
                decimal SubTotalAfterDiscount = order.OrderSubtotalExclTax - DiscountAmount;
                bool IsOrderHandlingApplicable = false;
                if(SubTotalAfterDiscount < 50)
                {
                    model.Shipping = 10;
                    IsOrderHandlingApplicable = true;
                }
                //Amounts
                model.Amount = order.OrderSubtotalExclTax - DiscountAmount + model.Shipping;  // IsOrderHandlingApplicable ? SubTotalAfterDiscount + 10 : SubTotalAfterDiscount;
                model.SalesTax = SalesTaxAmount;

                //Send To TaxJar
                (IsError, ErrorMessage) = await _taxJarService.CreateTransaction(model);
                if (IsError)
                {
                    var SiteData = _storeService.GetAllStoresAsync().Result.FirstOrDefault();
                    var mail = new MailMessage();
                    if(SiteData.Hosts == "fmsaccessories.com")
                        mail.To.Add("info@fmsaccessories.com");

                    mail.To.Add("mohit.tambe@vskysolutions.com");
                    mail.To.Add("sharvari.vardhamane@vskysolutions.com");
                    mail.To.Add("bhagyashri.pednekar@vskysolutions.com");

                    mail.Subject = $"Transaction Sync Failure To TaxJar For Order {order.Id}.";
                    mail.Body = $"Issue encountered during the transaction sync process with TaxJar. The transaction sync failed when placing an order, and as a result, the relevant data was not successfully transmitted to TaxJar.<br><b>Details of the issue:</b><br><b>Order Id: {order.Id}</b><br><b>Transaction On: {order.CreatedOnUtc}</b><br><b>{ErrorMessage}</b><br>This failure may impact tax reporting and compliance for this transaction.";
        ...
        private async Task<TaxJarAddress> GenerateTaxAddress(Address model)
        {
            var Data = new TaxJarAddress();

            if (model.CountryId != null)
            {
                var CountryData = await _countryService.GetCountryByIdAsync((int)model.CountryId);
                Data.Country = CountryData.TwoLetterIsoCode;
            }
            if (model.StateProvinceId != null)
            {
                var StateData = await _stateProvinceService.GetStateProvinceByIdAsync((int)model.StateProvinceId);
                Data.State = StateData.Abbreviation;
            }
            Data.City = model.City;
            Data.Street = model.Address1;
            Data.ZipCode = model.ZipPostalCode;
            return Data;
        }
            ...
            decimal orderSubTotal)
        {
            ...
            if (allDiscounts != null)
            {
                ...
                {
                    if (!_discountService.ContainsDiscount(allowedDiscounts, discount) && (await _discountService.ValidateDiscountAsync(discount, customer)).IsValid)
                    {
        ...
        public async Task<bool> SendOpenPymentNotification(Customer customer)
        {
            bool EmailStatus = false;
            System.Net.Mail.MailMessage mail = new System.Net.Mail.MailMessage();
            mail.To.Add(customer.Email);
            mail.Subject = "Order Placed for open payment";
            mail.Body = "Order placed using open payment method.";
            mail.IsBodyHtml = true;
        ...
        public virtual async Task<IList<string>> RefundAsync(Order order)
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    ...
                    //Mohit :- 10-11-2024 :- Update Redund Count by +1
                    order.RefundCount++;
        ...
        public virtual async Task PartiallyRefundOfflineAsync(Order order, decimal amountToRefund)
        {
            ...
            // Added By Yogesh Dt: 10/28/2024
            if (!String.IsNullOrEmpty(order.RefundPayQBOID))
                order.IsRefundPayModified = true;
