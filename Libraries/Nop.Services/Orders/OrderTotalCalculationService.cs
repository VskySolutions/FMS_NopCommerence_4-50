...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
            ...
            decimal discountAmountExclTax, decimal shippingTotalExclTax, decimal taxTotal)
        {
            ...
            updatedOrder.IsLessSubTotal = false;
            if ((subTotalExclTax - discountAmountExclTax) <= 50)
            {
                updatedOrder.IsLessSubTotal = true;
                total += 10;
            }
            ...
            foreach (var giftCard in await _giftCardService.GetAllGiftCardsAsync(usedWithOrderId: updatedOrder.Id))
            {
                ...
                var remainingAmount = (await _giftCardService.GetGiftCardUsageHistoryAsync(giftCard)).Where(history => history.UsedWithOrderId == updatedOrder.Id).Sum(history => history.UsedValue);
            ...
            bool includingTax)
        {
            ...
            //    var singleCustomer = cart.FirstOrDefault();
            //    if (singleCustomer == null) { }
            //    else if (singleCustomer != null && singleCustomer.EstimateCustReference == null) { }
            //    else
        ...
        public virtual async Task<(decimal? shoppingCartTotal, decimal discountAmount, List<Discount> appliedDiscounts, List<AppliedGiftCard> appliedGiftCards, int redeemedRewardPoints, decimal redeemedRewardPointsAmount)> GetShoppingCartTotalAsync(IList<ShoppingCartItem> cart, bool? useRewardPoints = null, bool usePaymentMethodAdditionalFee = true)
        {
            ...
            if (customer != null)
                paymentMethodSystemName = await _genericAttributeService.GetAttributeAsync<string>(customer, NopCustomerDefaults.SelectedPaymentMethodAttribute, store.Id);

            //subtotal without tax
            var (_, _, _, subTotalWithDiscountBase, _) = await GetShoppingCartSubTotalAsync(cart, false);

            //subtotal with discount
            orderTotal += subTotalWithDiscountBase;

            //shipping without tax
            var shoppingCartShipping = (await GetShoppingCartShippingTotalAsync(cart, false)).shippingTotal;
            if (shoppingCartShipping.HasValue)
                orderTotal += shoppingCartShipping.Value;
            ...
            if (usePaymentMethodAdditionalFee && !string.IsNullOrEmpty(paymentMethodSystemName))
            {
                ...
                orderTotal += (await _taxService.GetPaymentMethodAdditionalFeeAsync(paymentMethodAdditionalFee, false, customer)).price;
            }

            //tax
            //Mohit :- 10-07-2024 :- Skip if IsTaxExempt or BMW Dealer. 
            ////Skip cart tax calculations
            //if (IsCheckOut != null && (bool)IsCheckOut)
            //{
            
            bool IsBMWDealer = _workContext.GetCurrentCustomerAsync().Result.CustomerAccType == 6;
            if (!customer.IsTaxExempt && !IsBMWDealer)
            {
                //Custom TaxJar API Code - MT
                //Find Shipping Zipcode.
                if (customer.ShippingAddressId != null && customer.ShippingAddressId > 0)
                {
                    var TaxAddress = new TaxJarAddress();
                    ...
                    {
                        TaxAddress = await GenerateTaxAddress(address);
                        var (IsError, ErrorMessage, Total, _) = await _taxJarService.GetTaxOnOrder(TaxAddress, orderTotal);
                        if (IsError)
                            await _logger.ErrorAsync(ErrorMessage, null, customer);
            ...
            //Order handling
            if(subTotalWithDiscountBase <= 50)
                orderTotal += 10;

            //order total discount
            var (discountAmount, appliedDiscounts) = await GetOrderTotalDiscountAsync(customer, orderTotal);
            ...
            orderTotal = await AppliedGiftCardsAsync(cart, appliedGiftCards, customer, orderTotal);
